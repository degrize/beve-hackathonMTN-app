...
import { EntityArrayResponseType, SouscriptionService } from '../service/souscription.service';
...
export class SouscriptionComponent implements OnInit {
  ...
  trackId = (_index: number, item: ISouscription): number => this.souscriptionService.getSouscriptionIdentifier(item);
  ...
  protected onResponseSuccess(response: EntityArrayResponseType): void {
    ...
    this.souscriptions = dataFromBody;
  }

  protected fillComponentAttributesFromResponseBody(data: ISouscription[] | null): ISouscription[] {
    return data ?? [];
  }
  ...
  protected queryBackend(page?: number, predicate?: string, ascending?: boolean): Observable<EntityArrayResponseType> {
    ...
    return this.souscriptionService.query(queryObject).pipe(tap(() => (this.isLoading = false)));
  }
