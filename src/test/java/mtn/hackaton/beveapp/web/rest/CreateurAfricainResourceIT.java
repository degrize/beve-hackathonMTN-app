...
class CreateurAfricainResourceIT {
    ...
    @Mock
    private CreateurAfricainRepository createurAfricainRepositoryMock;

    @Autowired
    private CreateurAfricainMapper createurAfricainMapper;

    @Mock
    private CreateurAfricainService createurAfricainServiceMock;
    ...
    void getAllCreateurAfricainsWithEagerRelationshipsIsEnabled() throws Exception {
        when(createurAfricainServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restCreateurAfricainMockMvc.perform(get(ENTITY_API_URL + "?eagerload=true")).andExpect(status().isOk());

        verify(createurAfricainServiceMock, times(1)).findAllWithEagerRelationships(any());
    }

    @SuppressWarnings({ "unchecked" })
    void getAllCreateurAfricainsWithEagerRelationshipsIsNotEnabled() throws Exception {
        when(createurAfricainServiceMock.findAllWithEagerRelationships(any())).thenReturn(new PageImpl(new ArrayList<>()));

        restCreateurAfricainMockMvc.perform(get(ENTITY_API_URL + "?eagerload=false")).andExpect(status().isOk());
        verify(createurAfricainRepositoryMock, times(1)).findAll(any(Pageable.class));
    }
